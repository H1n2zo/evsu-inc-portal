...
// FIX: Payment upload form action now includes ?id= so POST retains the app ID.
...
$actionUrl  = $view->url('student/application_view.php') . '?id=' . (int)$app['id'];
?>
<body>
<div class="layout">
...
<main class="main-content">
  <div class="top-bar">
    <div>
      ...
      <p><?= $view->e($app['subject_name']) ?> — <?= $app['units'] ?> units — ₱<?= number_format($app['processing_fee'],0) ?></p>
    </div>
    <?php
  ...
  <div class="alert alert-danger">
    <strong>Rejected:</strong> <?= $view->e($app['rejection_reason']) ?>
  </div>
  <?php endif; ?>
  ...
  <?php if ((int)$app['current_step'] === 4 && $app['status'] === 'pending_payment'): ?>
  <div class="content-card" style="max-width:600px;">
    ...
    <div class="card-body">
      ...
      <!-- FIX: action includes ?id= so $_GET['id'] is available on POST -->
      <form method="POST" action="<?= $actionUrl ?>" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input class="form-input" type="file" name="payment_receipt" accept=".jpg,.jpeg,.png,.pdf" required style="height:auto;padding:8px 14px;">
