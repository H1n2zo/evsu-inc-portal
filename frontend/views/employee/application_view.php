...
// FIX: All four POST forms now include ?id= so $_GET['id'] is available on POST
...
$actionUrl  = $view->url('employee/application_view.php') . '?id=' . (int)$app['id'];
?>
<body>
<div class="layout">
    ...
    <div class="content-card">
      ...
      <div class="card-body" style="font-size:13.5px;">
        <table style="width:100%;border-collapse:collapse;">
          <?php $rows=[
            ['Student',        $view->e($app['full_name'])],
            ['Student ID',     $view->e($app['stu_id']??'—')],
            ['Subject',        $view->e($app['subject_name'])],
            ['Subject Code',   $view->e($app['subject_code'])],
            ['Units',          $app['units']],
            ['Processing Fee', '₱'.number_format($app['processing_fee'],0)],
            ['Semester',       $view->e($app['semester'])],
            ['School Year',    $view->e($app['school_year'])],
            ['Filed',          date('M d, Y H:i',strtotime($app['created_at']))],
          ]; ?>
          <?php foreach($rows as [$label,$val]): ?>
          <tr>
            <td style="color:var(--gray-400);padding:5px 0;width:40%;"><?= $label ?></td>
            <td style="padding:5px 0;font-weight:500;"><?= $val ?></td>
          </tr>
          <?php endforeach; ?>
        </table>
      </div>
      ...
      <div class="card-body" style="font-size:13px;">
        ...
        <div style="margin-bottom:0.875rem;padding-bottom:0.875rem;border-bottom:1px solid var(--gray-100);">
          <div style="display:flex;justify-content:space-between;">
            ...
            <span style="font-size:11.5px;color:var(--gray-400);"><?= date('M d, Y', strtotime($app['instructor_signed_at'])) ?></span>
          </div>
        ...
        <div style="margin-bottom:0.875rem;padding-bottom:0.875rem;border-bottom:1px solid var(--gray-100);">
          <div style="display:flex;justify-content:space-between;">
            ...
            <span style="font-size:11.5px;color:var(--gray-400);"><?= date('M d, Y', strtotime($app['dept_head_signed_at'])) ?></span>
          </div>
        ...
        <div>
          <div style="display:flex;justify-content:space-between;">
            ...
            <span style="font-size:11.5px;color:var(--gray-400);"><?= date('M d, Y', strtotime($app['registrar_signed_at'])) ?></span>
          </div>
    ...
    <div class="card-body">
      ...
      <form method="POST" action="<?= $actionUrl ?>">
        ...
        <div style="display:flex;gap:0.75rem;">
          ...
          <button type="submit" name="action" value="depthead_reject" class="btn-sm danger" style="height:42px;padding:0 1.25rem;"
                  onclick="document.getElementById('sigData').value='REJECTED';">
            ✕ Reject
          </button>
        </div>
      </form>
    ...
    <div class="card-body">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
        ...
        <div>
          ...
          <?php else: ?>
          <div style="border:1px dashed var(--gray-200);border-radius:var(--radius-sm);padding:3rem;text-align:center;color:var(--gray-400);">
  ...
  <div class="content-card">
    ...
    <div class="card-body">
      <div class="alert alert-gold" style="margin-bottom:1rem;">
        Ready to post final grade: <strong><?= $view->e($app['instructor_grade']??'—') ?></strong>
        for <strong><?= $view->e($app['subject_name']) ?></strong>
      </div>
      <form method="POST" action="<?= $actionUrl ?>">
        <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
